...
oc run tmp-pod -n storage-tester --image=dummy --restart=Never --overrides=$JSON
oc wait -n storage-tester --for=condition=Ready pod/tmp-pod --timeout=180s
...
NB_FILES=$(oc rsh -n storage-tester tmp-pod ls -1 /data-to-gather | wc -l)
oc delete pod -n storage-tester tmp-pod
