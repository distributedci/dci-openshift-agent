...
oc get all -n storage-tester -o wide
...
oc run tmp-rwo-pod -n storage-tester --image=dummy --restart=Never --overrides=$JSON
oc wait -n storage-tester --for=condition=Ready pod/tmp-rwo-pod --timeout=180s
RESULT=$(oc rsh -n storage-tester tmp-rwo-pod cat /data-to-gather/test.txt)
oc delete pod -n storage-tester tmp-rwo-pod
echo ""
...
TEST_TIME=$(oc get job/init-pv -n storage-tester --no-headers | awk '{print $4}')
echo ""
echo ">>>>>>> RESULTS:"
...
oc get all -n storage-tester -o yaml
echo ""