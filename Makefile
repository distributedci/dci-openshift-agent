...
install:
	...
	for hook in hooks/*.yml; do install -p -D -m 644 $$hook  $(BUILDROOT)$(SYSCONFDIR)/dci-openshift-agent/$$hook; done

	install -p -D -m 644 settings.yml $(BUILDROOT)$(SYSCONFDIR)/dci-openshift-agent/settings.yml

	for play in plays/*.yml; do install -p -D -m 644 $$play $(BUILDROOT)$(DATADIR)/dci-openshift-agent/$$play; done

	for script in plays/scripts/*; do install -p -D -m 755 $$script $(BUILDROOT)$(DATADIR)/dci-openshift-agent/$$script; done

	for role in $$(ls roles); do find roles/$$role -type f -exec install -v -p -D -m 644 "{}" "$(BUILDROOT)$(DATADIR)/dci-openshift-agent/{}" \; ; done

	cd common-roles; for role in $$(ls); do find $$role -type f -exec install -v -p -D -m 644 "{}" "$(BUILDROOT)$(DATADIR)/dci/roles/{}" \; ; done

	for plugin in action_plugins/*.py; do install -p -D -m 644 $$plugin $(BUILDROOT)$(DATADIR)/dci-openshift-agent/$$plugin; done
	...
	for cmd in extract-dependencies send-feedback test-runner; do install -v -p -D -m 755 $$cmd $(BUILDROOT)$(DATADIR)/dci-openshift-agent/; done
