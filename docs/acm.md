...
| dci_gitops_app_name              | Yes      | None       | The name of the ArgoCD Application to create.
| dci_gitops_app_namespace         | Yes      | None       | The namespace where the ArgoCD Application will be deployed.
| dci_gitops_repo                  | Yes      | None       | The repository to add to ArgoCD as an Application
| dci_gitops_repo_branch           | No       | HEAD       | The branch of the repo to use as a source <sup>1</sup>
| dci_gitops_app_path              | Yes      | None       | The path to the templates in the repository to render
| dci_gitops_app_target            | Yes      | None       | The path to the target directory in the repository, where the site-configs will be rendered and copied for the Application.
| dci_gitops_ssh_key               | No       | None       | The path to the private sshkey to use in the repository.

> [!IMPORTANT]
> <sup>1</sup> The target branch is automatically created by appending "-rendered" to the source branch name.
> For example, if source branch is "main", target branch will be "main-rendered".
> This target branch will contain the processed site-config files, whether or not Jinja2 templates are present in the
> original repository.
...
all:
  ...
  vars:
    ...
    dci_gitops_repo_branch: my-custom-branch
    dci_gitops_app_path: path/to/site-config-with-templates
    dci_gitops_app_target: path/to/target-rendered
