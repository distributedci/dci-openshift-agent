...
| dci_clusterinstance_namespace    | Yes      | None    | Namespace where the ClusterInstance will be created
...
But the dci-openshift-agent only requires 3, as Namespace is created automaticlaly. The other manifests are provided as templates to use the dci-openshift-agent capabilities to extract information from different sources like the Hub itself, DCI control server, the inventory and the pipeline file.
...
- name: openshift-ztp-clusterinstance
  ...
  ansible_extravars:
    ...
    dci_teardown_on_success: false
    # ClusterInstance
    dci_clusterinstance_namespace: "{{ cluster_name }}-ci"
