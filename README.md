...
- The topic or target version must be the an EUS release.
...
For example, to upgrade to a version where there's no path to it. Specify the intermediate version. `version_inter=4.11.5` or to use candidate versions use `version_inter=4.11.5-*`. Note the `-*`, this will help to include candidate versions in the search.
