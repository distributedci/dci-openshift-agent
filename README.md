...
* Please note the following settings:
 1. `prev_stages`: Indicates that this pipeline can only be executed after running and upgrade or OCP deployment, as those will provide the `kubeconfig`used to interact with the cluster during the upgrade
 1. `dci_main`: Instructs the agent to execute only the tasks related to a cluster upgrade
 1. `enable_perfor_addon`: Enables the mirroring of the Performance AddOn operator for the target OCP version
...
- Upgrades to the next support <major>.<minor>-<patch> release version according the (OCP update graph)[https://access.redhat.com/labsinfo/ocpupgradegraph].
- Extended Update Support (EUS) upgrades, if the current cluster version is EUS supported version. See (preparing EUS-EUS upgrade)[https://docs.openshift.com/container-platform/4.10///updating/preparing-eus-eus-upgrade.html].

### The upgrade process

Some of the relevant tasks executed during an cluster upgrade are listed below:

In disconnected environments:

  1. Create a DCI job that tracks the upgrade process
  1. The target OCP release is mirrored to the same images <registry>/<path> that was used for the cluster deployment. For EUS upgrades, the intermediate release images are also mirrored
  1. The ISO, rootfs, and other artifacts are mirrored to the same `webserver_url` used for the initial cluster deployment
  1. The Image Content Source Policies, and OCP signature for the new version are applied
  1. The cluster version is patched for the new target version based on upgrade
  1. The upgrade is executed and monitored for completion
  1. The Red Hat operators catalog for the new OCP version is pruned and mirrored according to the `enable_<operators>` variables defined in the pipeline file
  1. The current Red Hat operators catalog is replaced with the new one for the target version
  1. The operators upgrade is executed under the following conditions:
  1. The agent gathers information about current operators subscriptions and triggers an upgrade if:
     * There is a new CSV version available for the current installed operator
     * There is a new default channel available for the current installed operator that implies a new CSV availability
  1. Operators upgrade starts and is monitoring until it reaches the target CSV
  1. Cluster resources are validated to declare a cluster as successfully upgraded.
...
In connected environments:
  ...
  1. The cluster version is patched for the new target version based on upgrade
  1. The upgrade is executed and monitored for completion
  1. The operators upgrade is executed for operators not listed in the `operator_skip_upgrade` list
  1. The agent gathers information about current operators subscriptions and triggers an upgrade if:
     * There is a new CSV version available for the current installed operator
     * There is a new default channel available for the current installed operator that implies a new CSV version availability
  1. Operators upgrade starts and is monitoring until it reaches the target CSV
...
Openshift offers a way to facilitate the upgrade between two EUS versions. This allows upgrading from 4.8 to 4.10, or 4.10 to 4.12, etc. Detailed documentation is available in [preparing EUS-EUS upgrade](https://docs.openshift.com/container-platform/4.10/updating/preparing-eus-eus-upgrade.html)
...
- The topic or target version should be the an EUS release.

Transitioning between two OCP releases requires between and intermediate version. The variable `version_inter` allows specify the intermediate OCP version to use, instead of the one the upgrade path provides.

Let's say you want to upgrade to a version that there's no path for it yet. For example 4.10 to 4.12, then you will need to specify an intermediate version, for example: `version_inter=4.11.4`. Or maybe you need to land in a candidate version, then you could do that by doing: `version_inter=4.11.5-*`, please note the addition to `-*` this will help to include candidate versions in the search.

Notes:

* The upgrade process contains some times that may be dependant on the setting and facts defined by the `dci-openshift-agent` during a cluster deployment. So it is highly recommended to use it on cluster deployed by the agent.

* For nightlies releases, the upgrade process is forced because in most of the cases will not be upgrade paths available for the target release.

* For operators upgrades in disconnected environments, it is important to use the same `enable_<operator>` flag that was used for the initial cluster deployment. This is to allow to proper operators mirroring for the target OCP version.
