...
> NOTES:
 ...
 - The installer may require access to other endpoint (CDNs). The list above is for well know URLs that may be be subject to change.
...
- `pullsecret_file`: File with additional credentials in JSON format that will be used along with the ones provided by DCI job.
...
- Secrets are processed in the following order: Job secrets --> openshift_secret --> pull_secret_file
