...
In disconnected environments:
  ...
  1. The target OCP release is mirrored to the same images <registry>/<path> that was used for the cluster deployment. For EUS upgrades, the intermediate release images are also mirrored[^1].
  1. The ISO, rootfs, and other artifacts are mirrored to the same `webserver_url` used for the initial cluster deployment[^1].
  1. The Image Content Source Policies and OCP signature for the new version are applied[^1].
  1. The "cluster version" is patched for the new target version based on the calculated target version.
  1. The upgrade is executed and monitored for completion up to 150 minutes.
  1. The Red Hat operators catalog for the new OCP version is pruned and mirrored according to the `enable_<operators>` variables defined in the pipeline file[^1].
...
[^1]: Only in disconneted environments.
