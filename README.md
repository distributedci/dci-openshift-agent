...
| enable_ocs                         | False    | Boolean   | True       | Deploys the OCS Operator |
...
dci_operators:
  -
    ...
    catalog_source: "{{ opm_catalog_source_name }}"
    namespace: mirrored-redhat-operators
    operator_group_spec:
      targetNamespaces:
...
```
* In disconnected enviroments the catalog `mirrored-redhat-operators` contains the package manifests for the operators mirrored using the `opm_mirror_list` variable.
