...
| dci_prepare_cnf                    | False    | Boolean   | False      | Deploys Performance Addon and SRIOV operators. For performance addon confugiration it is recommended to also define a `performance_definition` |
...
The Agent manages the deployment of certain operators. At this time there is support for SRIOV, Performance Add On (PAO), HyperConverged Cluster Operator (HCO), ElasticSearch Operator, Cluster-Logging, OpenShift Container Storage (OCS), local-storage, and Advanced Cluster Management (ACM).
