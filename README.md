...
| dci_prepare_cnf                 | False    | Boolean | False                                                          | Configures the Performance Addon and SRIOV operators, Use the `dci_operators` variable to install the `sriov` operator. To complete the Performance Addon configuration it is also add required to set a `performance_definition`.
...
1. `enable_perf_addon`: Enables the mirroring of the Performance Add-On operator for the target OCP version. Performance add on has been integrated as part or NTO operator starting OCP 4.11.
...
* For disconnected environments, it's required to mirror the same operators originally installed in the cluster in order to allow the upgrade to the version used for thhe upgrade.
