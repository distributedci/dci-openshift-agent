...
- Each server must have reserved IP addresses for the baremetal NICs plus 2
  additional virtual IPs: API and Ingress
- Each server must have DNS setup for the API, wildcard applications. Please
...
2. [Jumpbox Configuration](#jumpbox-configuration)
3. [Copying the ssh key to your provisioner](#copying-the-ssh-key-to-your-provisioner)
...
Note: The initial copy of `dcirc.sh` is shipped as
`/etc/dci-openshift-agent/dcirc.sh.dist`.

Copy this to `/etc/dci-openshift-agent/dcirc.sh` to get started, then replace
inline some values with your own credentials.

From the web the [DCI web dashboard](https://www.distributed-ci.io), the
partner team administrator has to create a `Remote CI` in the DCI web
dashboard.

Copy the relative credential and paste it locally on the Jumpbox to
`/etc/dci-openshift-agent/dcirc.sh`.
...
mind that authentication is required to retrieve the images.
...
```ini
[all:vars]
prov_nic=eno1 # The NIC used for provisioning network on all nodes
pub_nic=eno2 # The NIC used for baremetal network on all nodes
...
;dnsvip=1.2.3.4 # Virtual IP for the cluster ingress
;masters_prov_nic=eno1 # The NIC used for provisioning network on master nodes if different
...
;cache_enabled=false # Enable some light caching on the provision host

;dci_disconnected=true # Activate disconnected mode in DCI OCP agent, requires you to set the following variables as well
;webserver_url="http://<jumpbox IP/DNS>:8080" # Must be reachable from the cluster
;disconnected_registry_auths_file=/path/to/auths.json # On the jumpbox
;disconnected_registry_mirrors_file=/path/to/trust-bundle.yml # On the jumpbox
;provision_cache_store="/path/to/qcow/cache" # On the jumpbox, must have enough space to hold your qcow images
;local_registry=local-registry:5000 # Registry that will mirror all container images
;local_repo=ocp4/openshift4 # Registry namespace
