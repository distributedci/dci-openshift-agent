...
The `opm_mirror_list` variable, controls the operators that will be mirrored if `dic_disconnected` is true. The Agent will take care of mirroring the
required operator images and prepared pruned catalog source in OCP.

The operators to mirror can be provided using a `Dictionary` or a `List`. Using a dictionary is recommended. Some examples are below:

Create a pruned catalog with specific channels. For the compliance-operator, all the channels will be included.
```yaml
opm_mirror_list:
  compliance-operator:
...
Another example of a pruned catalog will all operator's channels.
```yaml
opm_mirror_list:
  compliance-operator:
  file-integrity-operator:
...
Prune a list of operators. All operators channels are included.
```yaml
opm_mirror_list:
  - compliance-operator
  - file-integrity-operator
...
By default the catalog source name that can be used to create operator subscription is named `mirrored-redhat-operators`. Some operators may have other operators dependencies, for such cases the dependencies must be added to the list.
...
Important: To have a successful installation, it is required that the installation settings are according to the packages available in the corresponding catalog.
...
All the images available in the defined catalogs will be mirrored in `dci_disconnected` is `true` and a local image registry is defined.


Please see the [settings table](#ansible-variables) for the variables names to control the Operators installation.
...
The dci-openshift-agent can consume files generated by the `oc mirror` plugin. The agent uploads these files to the local registry, creates the catalog source, and applies the corresponding ICSPs. The catalog source is then used to deploy the operators enabled for testing.
