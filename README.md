...
| dci_must_gather_images          | False    | List    | []                                                             | Extra must-gather plugin images for the deployed cluster. Entries without "/" are short names or keywords (e.g. "ptp", "acm", "gitops") resolved from installed CSV relatedImages. Full image references (containing "/") are passed through and mirrored when `dci_local_registry` is set. Every collection also uses the cluster ImageStream `openshift/must-gather:latest`. Images must be reachable by the cluster directly or through mirrors.
| dci_hub_must_gather_images      | False    | List    |["acm", "gitops"]                                               | Extra must-gather plugin images for ACM Hub/ZTP. Same short-name, keyword, and full-ref rules as `dci_must_gather_images`. When `enable_acm` is true, the same list is also appended to the deployed-cluster gather. Full refs are mirrored when `dci_local_registry` is set. Images must be reachable by the cluster directly or through mirrors.
...
| dci_collect_hardware            | False    | Boolean | True                                                           | Collect hardware data.
| dci_hardware_tag                | False    | String  | latest                                                         | Tag used for the lshw container image in disconnected environments.
| dci_hardware_img_path           | False    | String  | /dci/lshw                                                      | Image path in the local registry for the lshw container used in disconnected hardware collection.
...
| dci_wipe_local_storage          | False    | Boolean | True                                                           | Wipe local ODF disks defined in `local_storage_devices`.
...
| enable_lvms                     | False    | Boolean | False                                                          | Configures LVM Storage operator to setup a local disk with LVM. Use `dci_lvms_dev_class_name`, `dci_lvms_dev_selector_paths`, and `dci_lvms_storage_class` to customize setup. See [setup_lvms](https://github/redhatci/ansible-collections-redhatci-ocp/roles/setup_lvms) for details.
...
| dci_pao_on_install              | False    | Bool    | True                                                           | Whether to apply the Performance Addon Operator during the installation of the cluster or after install. Used together with `performance_definition` (see below).
...
| eco_test_suites                 | False    | List    |                                                                | List of eco-gotest Tests to perform: ['ptp','sriov'].
...
| kubeadmin-password                              | Files   | Password assigned to the `kubeadmin` user [^1]                                            |
| kubeconfig                                      | Files   | Kubeconfig file to interact with the deployed OCP cluster [^1]                            |
...
| ocp_creds.txt                                   | Files   | A set of admin and non-admin credentials attached to an httpasswd identity provider [^1]  |
...
| image-sources.yaml                              | Files   | ImageDigestMirrorSets, ImageTagMirrorSets, and ImageContentSourcePolicies applied to the cluster |
