...
Some workloads like Migration Toolkit for Containers or Loki may require an object Object Storage provider. For such cases, a [Minio](https://min.io/) instance can be deployed on the OCP cluster by setting `true` to the `enable_minio` flag. In the DCI Openshift Agent integration, an initial bucket named `loki` is deployed and is used along with the [logging](#logging-stack) no details about an external Object provider are defined.

The following variables allow customizing the Minio deployment. Please see the [minio-setup](https://github.com/redhatci/ansible-collection-redhatci-ocp/tree/main/roles/setup_minio) role for additional details.
...
The workloads that require Object Storage, can use the `http://minio-service.minio:9000` endpoint and the default credentials set in the [minio-setup](https://github.com/redhatci/ansible-collection-redhatci-ocp/tree/main/roles/setup_minio) role to start shipping data to Minio.
