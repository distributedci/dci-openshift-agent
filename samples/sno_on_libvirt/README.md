...
The deployment consistes on a Single Cluster node playing the Master and Worked Node at same time. Single-node OpenShift deployment does not have an option to add additional hosts Single-node OpenShift isn’t highly-available. It explicitly does not assume zero downtime of the Kubernetes API.
...
These are the pseudo-oficial requirements to run:
- vCPU: 8
- RAM: 32 GB
- Storage: 120 GB
